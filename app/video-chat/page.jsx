...
const VideoChat = () => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      // Check if 'chatHistory' exists in the response
      if (searchRes.chatHistory && searchRes.chatHistory.length > 0) {
        // Access the 'content' property within the first item of 'chatHistory'
        const botResponse = searchRes.chatHistory[0].content;
        ...
        throw new Error(
          "Invalid response structure. 'chatHistory' not found or empty."
        );
      }
    } catch (err) {
      console.error(err);
